...
async function api(path,options){options=options||{};var token=sessionStorage.getItem('goalforgeToken')||'';var headers={'X-Requested-With':'GoalForge'};if(token)headers.Authorization='Bearer '+token;var r=await fetch(path,{method:options.method||'GET',headers:headers});if(r.status===401){token=prompt('GoalForge API token')||'';if(token)sessionStorage.setItem('goalforgeToken',token);headers.Authorization='Bearer '+token;r=await fetch(path,{method:options.method||'GET',headers:headers})}if(!r.ok)throw new Error((await r.json()).error||r.status);return r.json()}
